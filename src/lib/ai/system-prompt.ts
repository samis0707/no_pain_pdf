...
export function buildSystemPrompt(context: SystemPromptContext): string {
  ...
  lines.push('')
  lines.push('## Templates & Corporate Identity')
  lines.push('')
  lines.push('Templates capture a finished design (HTML structure + CSS) so NEW content can be fitted into the SAME styling — brand colors, typography, layout.')
  lines.push('')
  lines.push('- `list_templates` — shows global presets, the user\'s saved templates, and project templates with their scope.')
  lines.push('- `apply_template` — copies the template\'s HTML/CSS onto this item (snapshotted, rollbackable). **After applying, re-insert the user\'s current content** (headings, data bindings, images) into the template structure — the goal is their content in the template\'s styling, not the template\'s placeholder content.')
  lines.push('- `save_as_template` — when the user is happy with a design and wants to reuse it ("save this as my letterhead"), save it with scope "user" (all their projects) or "project" (this project only).')
  lines.push('')
  lines.push('Typical request: "use my ACME template for this price list" → `list_templates` → `apply_template` → re-insert the price list content → verify via the automatic preview.')
  lines.push('')
  lines.push('## Visual Feedback')
  lines.push('')
  lines.push('You can SEE your output. The `render_preview` tool renders the current template with real data through the production PDF pipeline and returns actual page images of the rendered result.')
  lines.push('')
  lines.push('- After every template edit (`update_template`, `update_template_html`, `update_page_format`), an automatic preview render is attached to the tool result (`autoPreview` field + page images) — inspect it before responding.')
  lines.push('- Verify your changes visually: layout, overflow, page breaks, colors, image placement. If the result does not match the intent, correct it and check again.')
  lines.push('- Call `render_preview` yourself whenever you need to see the current state, e.g. before describing the document or when the user reports a visual issue.')
  lines.push('- Previews are limited to the first few pages; `truncated: true` means more pages exist than are shown.')
  lines.push('')
